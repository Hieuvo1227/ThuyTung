...
export function DataTable<T>({
  ...
  return (
    <ScrollArea className="h-[calc(100vh-240px)] w-full rounded-xl bg-white dark:bg-gray-800">
      <CardContent className="p-2 md:p-4">
        <Table className="border-collapse [&_tr]:border-b [&_tr]:border-gray-200 dark:[&_tr]:border-gray-700">
          <TableHeader>
            <TableRow className="border-b border-gray-300 dark:border-gray-700">
              {columns.map((column, index) => (
                <TableHead
                  key={index}
                  className={`text-center font-bold text-xs md:text-sm py-2 ${column.className || ""}`}
                >
                  {column.header}
                </TableHead>
              ))}
              {actions && actions.length > 0 && (
                <TableHead className="text-right font-bold text-xs md:text-sm py-2">Actions</TableHead>
              )}
            </TableRow>
          </TableHeader>
              ...
              data.map((item, index) => (
                ...
                >
                  {columns.map((column, colIndex) => (
                    <TableCell
                      key={colIndex}
                      className={`text-center py-2 px-2 ${column.className || ""}`}
                    >
